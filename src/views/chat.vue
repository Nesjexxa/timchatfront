...
<div>
  ...
  <button v-on:click="sendMessage2Group">发送群聊消息</button><p/>
  <button v-on:click="TIMLogoutWithoutID">登出</button>
  <button v-on:click="TIMCreateGroup">创建群聊</button>
  <button v-on:click="TIMDismissGroup">删除群聊</button>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 将回调函数传递的event解析成分别的单个消息，存入msgList
    getNewMessage: function (event) {
